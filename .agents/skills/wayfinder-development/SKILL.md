...
```bash
./.infrastructure/scripts/mcp-wait.sh ./vendor/bin/spin run -T php php artisan wayfinder:generate --no-interaction
```
For form helpers, use `--with-form` flag:
```bash
./.infrastructure/scripts/mcp-wait.sh ./vendor/bin/spin run -T php php artisan wayfinder:generate --with-form --no-interaction
```
...
1. Run `./.infrastructure/scripts/mcp-wait.sh ./vendor/bin/spin run -T php php artisan wayfinder:generate` to regenerate routes if Vite plugin isn't installed
