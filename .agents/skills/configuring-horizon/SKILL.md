...
```bash
./.infrastructure/scripts/mcp-wait.sh ./vendor/bin/spin run -T php php artisan horizon:install
```
...
1. Run `./.infrastructure/scripts/mcp-wait.sh ./vendor/bin/spin run -T php php artisan horizon` and visit `/horizon`
...
- The metrics dashboard stays blank until `horizon:snapshot` is scheduled. Running `./.infrastructure/scripts/mcp-wait.sh ./vendor/bin/spin run -T php php artisan horizon` alone does not populate metrics.
